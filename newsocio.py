#************** Development on upgraded version starts on 15th Aug 2026*********
# ********** 9699 Sociology PYP Portal developed on 14th Aug 2026 ***********
import datetime
import io
import os
import fitz  # PyMuPDF
...
# ==========================================
st.set_page_config(
    page_title="9699 Sociology PYP Portal", 
    page_icon="👥",
    layout="wide"
)
...
# 6 Primary Folders for 9699 Sociology (AS & A Level components)
LOCAL_FOLDERS = {
    "p1_soc": "paper1_sociology",
    "p2_soc": "paper2_sociology",
    "p3_soc": "paper3_sociology",
    "p4_soc": "paper4_sociology",
...
def create_worksheet_docx(basket_items: list) -> io.BytesIO:
    ...
    doc.add_heading(f'PTES {SYLLABUS_CODE} Sociology Worksheet', level=1)
...
if 'has_auto_synced' not in st.session_state:
    ...
    with st.spinner("🚀 Waking up portal & auto-syncing Sociology files via Service Account..."):
        perform_bulk_sync()
...
st.subheader(f"👥 {SYLLABUS_CODE} Sociology PYP Resource Library")
...
with tab1:
    st.subheader("📚 Sociology Search ('AS' Level Papers (P1/P2)")
    
    selected_as_paper = st.selectbox(
        "Select Target Component Paper:", 
        options=["p1_soc", "p2_soc"],
        format_func=lambda x: "Paper 1 (The Family / Education)" if x == "p1_soc" else "Paper 2 (Theory and Methods)",
        key="select_as_paper"
    )

    as_kw = st.text_input(
        "Enter Keywords", 
        placeholder="e.g., Socialisation, Functionalism, Positivism, Patriarchy, Class, Participant Observation", 
        key="as_kw"
    )

    if st.button("Search Keyword", key="btn_search_as"):
        if as_kw.strip():
            with st.spinner("Scanning Sociology PDFs..."):
...
with tab2:
    st.subheader("🌍 Sociology Search ('A' Level Papers (P3/P4)")
    
    selected_a_paper = st.selectbox(
        "Select Target Component Paper:", 
        options=["p3_soc", "p4_soc"],
        format_func=lambda x: "Paper 3 (Education / Global Development)" if x == "p3_soc" else "Paper 4 (Media / Religion)",
        key="select_a_paper"
    )

    a_kw = st.text_input(
        "Enter Keywords", 
        placeholder="e.g., Secularisation, Globalisation, Media representation, Fundamentalism, Stratification", 
        key="a_kw"
    )
...
with tab3:
    ...
    if len(st.session_state.handout_basket) > 0:
        ...
        with st.spinner("Merging selected pages into Word Worksheet..."):
            ...
            target_filename = f"{SYLLABUS_CODE}_Sociology_Worksheet.docx"
        ...
        st.info("🛒 Your Cart is currently empty! Search AS or A Level Sociology tabs and click '➕ Add to Cart' to start building your worksheet.")
...
with tab5:
    ...
    if pwd_input and pwd_input == admin_pwd:
        ...
        with c1:
            st.link_button("📚 Paper 1 (p1_soc)", drive_links.get("p1_soc", "https://drive.google.com"), use_container_width=True)
            st.markdown("<br>", unsafe_allow_html=True)
            st.link_button("🔑 Mark Schemes (ms_p1_p2)", drive_links.get("ms_p1_p2", "https://drive.google.com"), use_container_width=True)
            
        with c2:
            st.link_button("📚 Paper 2 (p2_soc)", drive_links.get("p2_soc", "https://drive.google.com"), use_container_width=True)
            st.markdown("<br>", unsafe_allow_html=True)
            st.link_button("🔑 Mark Schemes (ms_p3_p4)", drive_links.get("ms_p3_p4", "https://drive.google.com"), use_container_width=True)
            
        with c3:
            st.link_button("📚 Paper 3 (p3_soc)", drive_links.get("p3_soc", "https://drive.google.com"), use_container_width=True)
            st.markdown("<br>", unsafe_allow_html=True)
            st.link_button("📚 Paper 4 (p4_soc)", drive_links.get("p4_soc", "https://drive.google.com"), use_container_width=True)
...
DEVELOPER_NAME = "Sociology & Computer Science Departments"
...
<div style="text-align: center; padding: 15px 0px; font-family: sans-serif;">
    ...
    <p style="margin: 5px 0 0 0; font-size: 0.85em; font-weight: 600; color: #384403;">💻 Developed for Sociology Department ({SYLLABUS_CODE})</p>
</div>
"""
